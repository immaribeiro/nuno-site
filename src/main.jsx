...
function normaliseEvent(event, index) { return { ...event, id: event.id || `${event.title || 'event'}-${index}`, title: event.title || 'Untitled event', topics: Array.isArray(event.topics) ? event.topics : [], dateValue: new Date(event.date) } }
...
function EventsPage() {
  const [events, setEvents] = useState([]); const [category, setCategory] = useState('All'); const [topics, setTopics] = useState(() => { try { const saved = JSON.parse(localStorage.getItem(TOPICS_STORAGE_KEY) || '[]'); return Array.isArray(saved) ? saved : [] } catch { return [] } }); const [topicInput, setTopicInput] = useState(''); const headingRef = useRef(null)
  useEffect(() => { fetch('./events.json').then((response) => { if (!response.ok) throw new Error('events unavailable'); return response.json() }).then((data) => setEvents(Array.isArray(data) ? data.map(normaliseEvent).filter((event) => !Number.isNaN(event.dateValue.getTime())).sort((a, b) => a.dateValue - b.dateValue) : [])).catch(() => setEvents([])) }, [])
  ...
  const filtered = events.filter((event) => eventMatches(event, category, topics)); const groups = groupEvents(filtered)
...
function App() {
  ...
  return <div className="min-h-screen bg-ink text-cream"><header className="site-header"><button className="wordmark" onClick={() => switchView('gallery')}>NUNO <span className="text-amber">/</span> IMMA</button><nav aria-label="Primary navigation"><button className={`nav-link ${view === 'gallery' ? 'is-active' : ''}`} onClick={() => switchView('gallery')} aria-pressed={view === 'gallery'}>The archive</button><button className={`nav-link ${view === 'events' ? 'is-active' : ''}`} onClick={() => switchView('events')} aria-pressed={view === 'events'}>Events</button></nav></header>{view === 'gallery' ? <Gallery photos={photos} quotes={quotes} onPhoto={setActive} /> : <EventsPage />}<footer className="border-t border-cream/15 px-5 py-12 sm:px-8 lg:px-12"><div className="mx-auto flex max-w-[1440px] items-end justify-between"><p className="label text-cream">Nuno + Imma</p><p className="label text-right text-cream-dim">A small archive of us<br />2024 — now</p></div></footer><div className="grain" />{active !== null && <Lightbox photos={photos} index={active} onClose={() => setActive(null)} onChange={setActive} />}</div>
}
