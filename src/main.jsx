...
function EventsPage() {
  const [events, setEvents] = useState([]); const savedTopics = useRef(readStorage(TOPICS_STORAGE_KEY)); const [topics, setTopics] = useState(savedTopics.current); const [focusTopics, setFocusTopics] = useState([]); const [sharedPins, setSharedPins] = useState([]); const [personalPins, setPersonalPins] = useState(() => readStorage(PINNED_STORAGE_KEY).map(normalisePin)); const [category, setCategory] = useState('All'); const [topicInput, setTopicInput] = useState(''); const [refreshing, setRefreshing] = useState(false); const [refreshMsg, setRefreshMsg] = useState(''); const headingRef = useRef(null)
  ...
  const refreshData = async () => { if (refreshing) return; setRefreshing(true); setRefreshMsg(''); try { const response = await fetch('/api/refresh', { method: 'POST', headers: { Authorization: `Bearer ${SITE_TOKEN}` } }); const data = await response.json(); if (!response.ok) { setRefreshMsg('Erro ao atualizar'); return } const fresh = await fetch(`./events.json?t=${Date.now()}`).then((r) => r.ok ? r.json() : []); if (Array.isArray(fresh)) setEvents(fresh.map(normaliseEvent).filter((event) => !Number.isNaN(event.dateValue.getTime())).sort((a, b) => a.dateValue - b.dateValue)); setRefreshMsg('Atualizado ✓') } catch { setRefreshMsg('Erro ao atualizar') } finally { setRefreshing(false) } }
  ...
  return <main id="events" className="events-page"><div className="mx-auto max-w-[1440px] px-5 py-16 sm:px-8 sm:py-24 lg:px-12 lg:py-32">
    <div className="mb-12 max-w-[42rem]"><p className="label mb-5 text-amber">03 / OUT IN THE WORLD</p><h1 ref={headingRef} tabIndex="-1" className="font-display text-[clamp(3rem,8vw,6rem)] font-semibold leading-[.94] tracking-[-.025em]">Events</h1><p className="mt-6 font-body text-base leading-[1.55] text-cream-muted">Places to go, things to hear, and a few reasons to leave the house.</p><button type="button" className="us-action mt-6" onClick={refreshData} disabled={refreshing}>{refreshing ? 'A atualizar…' : 'Update now ↻'}{refreshMsg && <span className="ml-3 label text-cream-dim">{refreshMsg}</span>}</button></div>
...
function NewsPage() {
  ...
  const [news, setNews] = useState([]); const [topics, setTopics] = useState(savedTopics.current); const [category, setCategory] = useState('All'); const [topicInput, setTopicInput] = useState(''); const [refreshing, setRefreshing] = useState(false); const [refreshMsg, setRefreshMsg] = useState(''); const headingRef = useRef(null)
  ...
  useEffect(() => { headingRef.current?.focus() }, [])
  const addTopic = (event) => { event.preventDefault(); const topic = topicInput.trim().replace(/\s+/g, ' '); if (topic && !topics.some((item) => item.toLowerCase() === topic.toLowerCase())) setTopics([...topics, topic]); setTopicInput('') }
  const refreshData = async () => { if (refreshing) return; setRefreshing(true); setRefreshMsg(''); try { const response = await fetch('/api/refresh', { method: 'POST', headers: { Authorization: `Bearer ${SITE_TOKEN}` } }); const data = await response.json(); if (!response.ok) { setRefreshMsg('Erro ao atualizar'); return } const fresh = await fetch(`./news.json?t=${Date.now()}`).then((r) => r.ok ? r.json() : []); if (Array.isArray(fresh)) setNews(fresh.map(normaliseNews).filter((item) => !Number.isNaN(item.dateValue.getTime())).sort((a, b) => b.dateValue - a.dateValue)); setRefreshMsg('Atualizado ✓') } catch { setRefreshMsg('Erro ao atualizar') } finally { setRefreshing(false) } }
  const clearFilters = () => { setCategory('All'); setTopics([]) }
  ...
  return <main id="news" className="events-page news-page"><div className="mx-auto max-w-[1440px] px-5 py-16 sm:px-8 sm:py-24 lg:px-12 lg:py-32">
    <div className="mb-12 max-w-[42rem]"><p className="label mb-5 text-amber">04 / WHAT'S HAPPENING</p><h1 ref={headingRef} tabIndex="-1" className="font-display text-[clamp(3rem,8vw,6rem)] font-semibold leading-[.94] tracking-[-.025em]">News</h1><p className="mt-6 font-body text-base leading-[1.55] text-cream-muted">A concise read on the places, people, and ideas making noise.</p><button type="button" className="us-action mt-6" onClick={refreshData} disabled={refreshing}>{refreshing ? 'A atualizar…' : 'Update now ↻'}{refreshMsg && <span className="ml-3 label text-cream-dim">{refreshMsg}</span>}</button></div>
